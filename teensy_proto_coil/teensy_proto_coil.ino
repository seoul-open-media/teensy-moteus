...
Adafruit_BNO055 bno1 = Adafruit_BNO055(-1, 0x29, &Wire);

double ZERO_1 = 5.5;
double ZERO_2 = 0.0;

int status = 0; // zero = 0, left = -1, right = 1;
double basic_angle = 4.0;
double angle_error = 3.0;
...
void loop() {
  ...
  //   // Serial.println("");
  // }
  // int ch1 = pulseIn(15, HIGH, 25000);
  // ch1 = map(ch1, 1255, 1755, 0, 251);
  // int ch2 = pulseIn(14, HIGH, 25000);
  // ch2 = map(ch2, 1260, 1760, 0, 255);
  // int ch3 = pulseIn(43, HIGH, 25000);
  // ch3 = map(ch3, 1020, 2020, 0, 249);
  // int ch4 = pulseIn(44, HIGH, 25000);
  // ch4 = map(ch4, 1260, 1760, 0, 258);
  // int ch5 = pulseIn(42, HIGH, 25000);
  // ch5 = map(ch5, 2000, 1000, -1, 1);
  //   double speed = 0.01;
  //   double error = 0.5;
  // if(ch5 != ch5_temp && ch5 >= -1 && ch5 <= 1){
  //    target2walk1(ZERO_1, speed, error,0);
  //     target2walk2(ZERO_2, speed, error,0);
  //     stay2walk(50);
  ...
  //   //   target2walk1(ZERO_1, speed, error,0);
  //   //   target2walk2(ZERO_2, speed, error,0);
  //   //   target2walk1(ZERO_1 - basic_angle, speed, error,0);
  //   //   target2walk2(ZERO_2, speed, error,0);
  //   //   stay2walk(50);
  ...
  //   //   target2walk2(ZERO_2, speed, error,0);
  //   //   target2walk1(ZERO_1, speed, error,0);
  //   //   target2walk2(ZERO_2 - basic_angle, speed, error,0);
  //   //   stay2walk(50);
  ...
  //       status = -1;
  //     }else if(status == -1){
  //       target2walk2(ZERO_2 - (basic_angle - angle_error), speed, error,0);
  //       stay2walk(50);
  //       Serial.println("right set.");
  //       status = 1;
  //     }else if(status == 1){
  ...
  //     if(status == 0){
  //       target2walk1(ZERO_1 - (basic_angle - angle_error), speed, error,0);
  //       stay2walk(50);
  //       Serial.println("left set.");
  //       status = -1;
  //     }else if(status == -1){
  ...
  //   Serial.println("anti-bow");
  //   bow2walk2(1, speed);
  //   return;
  // }else{
  //   if(ch4 > 153){
  //     Serial.println("right head");
  //     hey2walk2(1, -1*speed/2);
  //   }else if(ch4 < 103){
  //     Serial.println("left head");
  //     hey2walk2(1, speed/2);
  //   }else{
  //     free2walk2();
  //   }
  ...
  if (Serial.available()) {
    ...
    if (firstValue.toInt() == 5) {
      chezo2walk1(secondValue.toFloat(), 0.01, 0.5, 0);
      return;
    }
    if (firstValue.toInt() == 6) {
      chezo2walk2(secondValue.toFloat(), 0.01, 0.5, 0);
      return;
    }
...
void chezo2walk1(double target, double speed, double error, double zero) {
  while (1) {
...
void chezo2walk2(double target, double speed, double error, double zero) {
  while (1) {
...
void diff2walk(int motor, double pos) {
  ...
  position_cmd1.velocity = 0.2;
  position_cmd2.velocity_limit = 2.0;
  position_cmd2.accel_limit = 2.0;
  position_cmd2.maximum_torque = 4.0;
  position_cmd2.velocity = 0.2;
  if (motor == 1) {
    Serial.println(pos);
    double origin1 = moteus1.last_result().values.position;
    double origin2 = moteus2.last_result().values.position;
...
void bow2walk2(int motor, double pos) {
...
void hey2walk2(int motor, double pos) {
...
void run2walk2_coil(int motor, double pos) {

  position_cmd1.velocity_limit = 5.0;
  position_cmd1.accel_limit = 3.0;
  position_cmd1.maximum_torque = 10.0;
  position_cmd1.velocity = 0.5;
  position_cmd2.velocity_limit = 5.0;
  position_cmd2.accel_limit = 3.0;
  position_cmd2.maximum_torque = 10.0;
  position_cmd2.velocity = 0.5;
  if (motor == 1) {
    Serial.println(pos);
    double origin1 = moteus1.last_result().values.position;
    double origin2 = moteus2.last_result().values.position;
